...
- 로그인 후 메뉴 > **프로필 설정**에서 한 화면에서 전부 관리: 닉네임, 색상, 프로필 사진, **표정 이미지 여러 장 등록**, 상태(근무중 등)+한마디+위치, 배경화면, 트위터 프로필 링크, **배경음악(유튜브 링크)**, 시간 표시 on/off, 방문자 사진 첨부 허용 on/off
- 배경음악을 등록해두면 워크스페이스 바 오른쪽에 재생/정지 버튼이 뜸. 유튜브 영상을 그대로 재생하는 방식이라 음원 파일을 직접 올리는 게 아니고, 브라우저 정책상 버튼을 한 번 눌러야 소리가 남(자동재생 안 됨)
